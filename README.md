...
​
```sh
npx create-react-app react-context-countries
```
​
En este ejercicio tendrás que crear una aplicación en React que permita listar todas las naciones que nos devuelve la API de `restcountries.com`.
​
Esta aplicación va a tener dos páginas: El index `/` y el detalle de una nación en `/details`
​
Para utilizar react router tienes que instalar react router dom v6
​
```sh
